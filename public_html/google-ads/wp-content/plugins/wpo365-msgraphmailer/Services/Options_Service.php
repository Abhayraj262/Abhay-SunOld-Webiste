...
if (!class_exists('\Wpo\Services\Options_Service')) {
    ...
    class Options_Service
    {
        ...
        public static function get_global_string_var($name, $log = true)
        {
            ...
            if (in_array($name, $aad_options)) {
                return self::get_aad_option($name);
            }
        ...
        public static function get_options($to_camel_case = true)
        {
            try {
                return $to_camel_case
                    ? self::to_camel_case($GLOBALS['WPO_CONFIG']['options'])
        ...
        public static function is_wpo365_configured()
        {
            ...
            /**
             * @since 27.0  Consider WPO365 configured if wp-config.php is used instead.
             */

            if (!empty(Wp_Config_Service::get_single_idp()) || !empty(Wp_Config_Service::get_multiple_idps())) {
                $request->set_item('is_wpo365_configured', 1);
                return true;
            }
        ...
        public static function get_aad_option($option_name)
        {
            ...
            if (false === $wpo_aad) {
                Log_Service::write_log('ERROR', sprintf('%s -> The AAD options have not been cached', __METHOD__));
                return '';
            }

            return !empty($wpo_aad[$option_name]) ? $wpo_aad[$option_name] : '';
        }
        ...
        public static function get_mail_option($option_name)
        {
            ...
            if (false === $wpo_mail && !empty($GLOBALS['WPO_CONFIG']['options'])) {
                Log_Service::write_log('ERROR', sprintf('%s -> The Graph Mailer options have not been cached', __METHOD__));
                return '';
            }

            return !empty($wpo_mail[$option_name]) ? $wpo_mail[$option_name] : '';
        }
