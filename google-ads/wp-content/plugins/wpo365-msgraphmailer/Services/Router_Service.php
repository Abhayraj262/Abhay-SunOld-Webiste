...
if (!class_exists('\Wpo\Services\Router_Service')) {
    ...
    class Router_Service
    {
        ...
        {
            // Skip if there is no authentication response to process or the admin has disabled SSO and the Graph Mailer is also disabled
            if ((!$is_oidc_response && !$is_saml_response) || (Options_Service::get_global_boolean_var('no_sso', false) && !Options_Service::get_global_boolean_var('use_graph_mailer', false))) {
